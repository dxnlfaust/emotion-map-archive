...
function init() {
  ...
  const colors = [
    new THREE.Color(0xC469C8),
    new THREE.Color(0xD096E3),
    new THREE.Color(0x5E6AC0),
    new THREE.Color(0x7986D4),
    new THREE.Color(0xF6CCA2),
    new THREE.Color(0xEBB656)
  ];
...
function animate() {
  ...
  if (zoomTarget && zoomProgress < 1) {
    zoomProgress += 0.02;
    camera.position.lerpVectors(defaultCameraPosition, zoomTarget, easeOutCubic(zoomProgress));
    if (zoomProgress >= 1) {
...
function resumeAnimation() {
  ...
  // Restore drift to original scale
  for (const sphere of spheres) {
    for (let i = 0; i < sphere.driftVectors.length; i++) {
      sphere.driftVectors[i].multiplyScalar(10); // restore drift
    }
  }

  // Clean up secondary overlays
